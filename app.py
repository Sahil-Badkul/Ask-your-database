    ...
    with st.chat_message("assistant"):
        ...
        with st.spinner("Generating SQL..."):
            ...
            try:
                ...
                result = execute_query(final_sql)
                if result is None:
                    raise ValueError("Query returned no results or invalid result structure.")
                ...
                if isinstance(result, tuple):
                    ...
                    if len(columns) == df.shape[1]:
                        ...
                        st.warning(
                            "Column count does not match query results; showing raw data."
                        )
                    st.dataframe(df)
                    turn["dataframe"] = df
                elif isinstance(result, int):
                    st.write(f"Query executed successfully. {result} rows affected.")
                    turn["dataframe"] = None
                else:
                    raise ValueError("Unexpected result type from database query.")
